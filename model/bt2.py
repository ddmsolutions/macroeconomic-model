...
def fitw(d):
    ...
    m['w']=ols(d.pay,X,EX)
    return m
def simw(m,d,H,lo=None,lfx=None,lhe=None,wage=True):
    ...
    pc=(m['pcw'] if wage else m['pc']).params; is_,tr,ok,w=m['is'].params,m['tr'].params,m['ok'].params,m['w'].params
    for p in idx:
        gap=is_.gap1*e.gap.shift(1)[p]+is_.gap2*e.gap.shift(2)[p]+RG*(e.i-e.cpi-e.rstar).shift(2)[p]+OILD*e.doil.shift(1)[p]
        e.loc[p,'gap']=gap
        pw=w.const+w.w1*e.pay.shift(1)[p]+w.cpi1*e.cpi.shift(1)[p]+w.ug1*(e.u-e.ustar).shift(1)[p]+w.dinact*e.dinact.shift(1)[p]
        e.loc[p,'pay']=pw
        x=pcX(e).loc[p]
        dp=0.5+(pc[[c for c in pc.index if c!='wg1']]*x[[c for c in pc.index if c!='wg1']]).sum()+(pc.get('wg1',0)*(e.pay.shift(1)[p]-3.5))
...
if __name__=='__main__':
    ...
    for wage in (False,True):
        e=simw(m,d,H,g('lo'),g('lfx'),g('lhe'),wage)
        print('wage' if wage else 'base', 'CPI', e.loc[idx,'cpi'].round(1).tolist(), 'pay', e.loc[idx,'pay'].round(1).tolist())
    ...
    for T in origins:
        ...
        for wage in (False,True):
            ...
            for h in (4,8):
                ...
                rows.append({'T':T,'h':h,'wage':wage,'err':e.loc[tp,'cpi']-act.loc[tp,'cpi'],'perr':e.loc[tp,'pay']-act.loc[tp,'pay'],'pnaive':act.loc[T,'pay']-act.loc[tp,'pay']})
    R=pd.DataFrame(rows); rm=lambda x: np.sqrt((x**2).mean())
    print(R.groupby(['wage','h']).err.apply(rm).round(3))
    print('pay model vs naive', R[R.wage].groupby('h').perr.apply(rm).round(2).to_dict(), R[R.wage].groupby('h').pnaive.apply(rm).round(2).to_dict())
